...
contract ERC721 is Context, ERC165, IERC721 {
    ...
    // Mapping from token ID to staking address
    mapping (uint256 => address) public _tokenStakings;
    
     // Mapping from token ID to staking block number
    mapping (uint256 => uint256) private _tokenStakingBlockNumber;
    ...
    mapping (address => mapping (address => bool)) private _operatorApprovals;
     ...
     // Mapping from token ID to json metadata
    ...
    function mintAll(string memory metadata) public {
        require(_msgSender() == deployer, "Only deployer can mint token.");
        require(isMintAll == false, "Already mint all.");
        
        for (uint256 i = 1; i <= copies; i++) {
            _tokenOwner[i] = deployer;
    ...
    /**
     * @dev set fee distribution.
     * @param _fee_receivers address[] fee receiver
     * @param percentage uint256[] fee percentage
     */
    function setFeeDistributionPercentage(address[] memory _fee_receivers, uint256[] memory percentage) public {
        ...
        for (uint256 i = 0; i < _fee_receivers.length; i++) {
    ...
    function checkFeeDistributionPercentage(address[] memory _fee_receivers, uint256[] memory percentage) public {
        require(_fee_receivers.length == percentage.length, "ERC721: fee receiver count and percentage count should be same value.");
        require(_fee_receivers.length == fee_receivers.length, "ERC721: not registered distribution.");
        
        for (uint256 i = 0; i < _fee_receivers.length; i++) {
            require(percentage[i] <= 100, "ERC721: percentage should not be greater than 100.");
            require(fee_distribution[_fee_receivers[i]] == percentage[i], "ERC721: not registered distribution.");
        }
    }
    
    /**
     * @dev clear fee distribution.
     */
    function clearFeeDistribution() public {
        require(deployer == _msgSender(), "ERC721: caller is not the deployer.");
    ...
    function safeMint(address to, uint256 tokenId, string memory metadata) public {
        require(_msgSender() == deployer, "Only deployer can mint token.");
     ...
     */
    function _safeMint(address to, uint256 tokenId, string memory metadata) internal {
        _safeMint(to, tokenId, "", metadata);
    }
     ...
     * @param _data bytes data to send along with a safe transfer check
     */
    function _safeMint(address to, uint256 tokenId, bytes memory _data, string memory metadata) internal {
    ...
    function _transferFrom(address from, address to, uint256 tokenId) internal {
        ...
        require(_tokenStakings[tokenId] == address(0), "ERC721: the token has been staked.");
    ...
    function stake(uint256 tokenId, address _stakingAddress) public {
        require(_isApprovedOrOwner(_msgSender(), tokenId), "ERC721: the only owner can stake the token.");
        require(_tokenStakings[tokenId] == address(0), "ERC721: the token has been already staked.");
        
        _tokenStakings[tokenId] = _stakingAddress;
        _tokenStakingBlockNumber[tokenId] = block.number;
    }
    
    function clearStake(uint256 tokenId) public {
        require(_tokenStakings[tokenId] == _msgSender(), "ERC721: the only staking owner can release the stake.");
        
        _tokenStakings[tokenId] = address(0);
        _tokenStakingBlockNumber[tokenId] = 0;
    }
    
    function getStakingBlockNumber(uint256 tokenId) public returns(uint256) {
        return _tokenStakingBlockNumber[tokenId];
    }
}